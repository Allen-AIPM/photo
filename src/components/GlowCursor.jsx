...
void main() {
  ...
  for (int i = 0; i < MAX_POINTS - 1; i++) {
    if (float(i) >= uPointCount - 1.0) break;
    float index = float(i);
...
const GlowCursor = ({
  ...
  useDeferredWebGL(({Mesh, Program, Renderer, Triangle}) => {
    ...
    const render = now => {
      raf=0;
      if (destroyed || document.hidden || !initialized) return;
      ...
      if (!document.hidden && (fade > .001 || !config.idleFade)) renderer.render({ scene: mesh });
      else gl.clear(gl.COLOR_BUFFER_BIT);
      if (!destroyed && (fade > .001 || fadeTarget > 0)) raf = requestAnimationFrame(render);
    };

    const visibility=()=>{if(document.hidden){cancelAnimationFrame(raf);raf=0;pointerInside=false;initialized=false;fade=0;gl.clear(gl.COLOR_BUFFER_BIT)}};
